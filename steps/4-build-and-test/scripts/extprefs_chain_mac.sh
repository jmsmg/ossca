...
autoninja -C out/Default obj/extensions/browser/browser_sources/extension_prefs.o obj/chrome/browser/extensions/unit_tests/extension_prefs_unittest.o >> "$L/extprefs_obj.log" 2>&1
echo "OBJ=$?" >> "$L/extprefs_done.marker"
if grep -q 'Build Succeeded' "$L/extprefs_obj.log"; then
  autoninja -C out/Default unit_tests > "$L/extprefs_build.log" 2>&1
