...
# media_unittests + viz_unittests + google_apis_unittests 를 한 번에 빌드(-j 4)하고 각 필터를 실행.
...
nice -n 5 autoninja -j 4 -C out/Default media_unittests viz_unittests google_apis_unittests > "$L/abec_build.log" 2>&1
echo "BUILD=$?" >> "$M"
