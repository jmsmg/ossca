#!/bin/bash
# 380105415 kMultipleLcppKeyInitiatorOriginFix 킬스위치 제거 검증 (Mac). unit_tests(-j 4, sync 후 첫 빌드) → LCPP 테스트.
...
nice -n 5 autoninja -j 4 -C out/Default unit_tests > "$L/lcpp_build.log" 2>&1
echo "BUILD=$?" >> "$M"
