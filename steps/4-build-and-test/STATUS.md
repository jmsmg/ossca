...
| (V·Z·X·W 통합, **리눅스**) | `out/cros` 증분: `ash_unittests`+`unit_tests`+`remoting_unittests`, 브랜치 `verify-vzxw` · gn check 4타깃 ✓ | `verify_vzxw_linux.sh` | `vzxw_{gn,gncheck,build}.log`, `vzxw_{ash,remoting,unit}_test.log`, `vzxw_done.marker` | ✅ 09-24 20:39 KST — 증분 962스텝(첫 실행에서 완료, 재실행 시 0스텝) · gn check 4타깃 ✓ · **Z 5/5 · V 45/45 · X+Y 32/32** (대조군 7·46·33에서 지운 테스트만큼 감소, 지운 테스트 이름 0건). 첫 실행 마커의 `BUILD=FAILED`는 실행 중 러너 파일을 고쳐 쓴 탓(코드 무관) |
