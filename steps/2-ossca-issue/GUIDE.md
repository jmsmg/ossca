...
## 템플릿 실물과 맞추기 (2026-09-07 확인)

`~/contributions/.github/ISSUE_TEMPLATE/crbug-에서-찾은-이슈-등록.md`가 채워주는 것과,
실제로 등록된 이슈(#369)가 어떤 모양인지의 차이:

| 항목 | 템플릿이 채워줌 | 등록할 때 |
|---|---|---|
| 제목 | `[crbug 번호] 이슈 제목` | 그대로 — crbug 영문 제목 |
| `## Issue` 헤딩 | 있음 | **지운다** (#369는 `**Bug**`로 바로 시작) |
| **Bug**/**Documents**/**Description**/**References** 뼈대 | 있음 (`- .` 만) | 드래프트 내용으로 채운다 |
| 맨 아래 `---` + 「기여 기록하기 가이드」 안내문 | 있음 | **지운다** (#369에 없음) |
| 라벨 `2026` · `chromium-issues` | 자동 | 그대로 |
| 라벨 `self-issues` | **안 붙음** | 직접 발굴이면 **수동 추가** (#369·#342 모두 붙어 있음) |
| Assignees | 비어 있음 | **수동 self-assign** |

`blank_issues_enabled: false`라 빈 이슈는 못 만든다. 템플릿을 반드시 거친다.

**Status는 진행 상태에 맞춰 놓는다** — 등록이 밀려서 CL이 이미 Gerrit에 올라가 있으면
기본값 `멘티 작업 진행 중`이 아니라 **`gerrit 리뷰 중`**으로 바로 놓아야 실제와 맞는다.
...
- (프로그램 체크리스트에 «docs면 멘토를 리뷰어에, 그 외엔 CC에» 항목이 있으나 **따르지 않는다** — 5단계 GUIDE 참고)
